...
// 		CV_PROFILE("solvePnPRansac",cv::solvePnPRansac(ppcloud, imgPoints, K, distortion_coeff, rvec, t, true, 1000, 0.006 * maxVal, 0.25 * (double)(imgPoints.size()), inliers, CV_EPNP);)
		CV_PROFILE("solvePnPRansac",cv::solvePnPRansac(ppcloud, imgPoints, K, distortion_coeff, rvec, t, true, 1000, 0.006 * maxVal, 0.99, inliers, CV_EPNP);)
...
void MultiCameraPnP::AdjustCurrentBundle() {
	...
	pointcloud_beforeBA = pcloud;// from 2 view constructure
	...
	K = cam_matrix; //cam_matrix is after ajust
	Kinv = K.inv(); 
...
//#pragma omp parallel for
	for (int _i=0; _i < imgs.size() - 1; _i++)
	{
		for (unsigned int _j=_i+1; _j < imgs.size(); _j++) {
			...
			GetFundamentalMat( imgpts[older_view], //pre-image
				imgpts[working_view], //current -image
...
void MultiCameraPnP::RecoverDepthFromImages() {
	...
	GetBaseLineTriangulation();//Get an initial 3D point cloud from 2 views only
	AdjustCurrentBundle();
